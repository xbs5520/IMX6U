...
void int_Init(void)
{
    GIC_Init();                         // Initialize GIC (Generic Interrupt Controller)
    system_irqtable_init();          // Initialize Interrupt handler table
    __set_VBAR((uint32_t)0x87800000);   // Inetrrupt handler table offset
}
